...
cur = conn.cursor()
cur.execute("DROP TABLE board_Product")
cur.execute("CREATE TABLE board_Product(name VARCHAR(255) PRIMARY KEY, price INT, board_ddr VARCHAR(255), board_atx VARCHAR(255), board_socket VARCHAR(255), url VARCHAR(255), image VARCHAR(255))")
...
while True:
    ...
    driver.get(url = 'https://search.danawa.com/dsearch.php?query=%EB%A9%94%EC%9D%B8%EB%B3%B4%EB%93%9C&originalQuery=%EB%A9%94%EC%9D%B8%EB%B3%B4%EB%93%9C&previousKeyword=cpu&excludeKeyword=%ED%95%B4%EC%99%B8%EA%B5%AC%EB%A7%A4%7C%EC%A4%91%EA%B3%A0%7C%EB%A0%8C%ED%83%88&isShortcut=true&cate_c1=861&cate_c2=875&volumeType=vmvs&page='+str(i)+'&limit=40&sort=saveDESC&list=list&boost=true&addDelivery=N&attributeOption=751|506|2459|OR,751|506|2464|OR,751|506|2460|OR,751|506|2461|OR,751|506|33385|OR&mode=simple&recommendedSort=Y&defaultUICategoryCode=112751&defaultPhysicsCategoryCode=861%7C875%7C968%7C0&defaultVmTab=925&defaultVaTab=0&isZeroPrice=N&tab=main')
            ...
            for li in lis:
                try:
                    ...
                    if name:                    
                        ...
                        a4 = '/'.join(a3)
                        board_ddr = re.sub(r"[^0-9/^DDR$]", "", a4)

                        b1 = re.sub(r"[^-/^A-Z/^(]", "", spec)
                        b2 = b1.split('/')
                        b3 = [word for word in b2 if 'TX(' in word]
                        b4 = '/'.join(b3)
                        board_atx = re.sub(r"[^-/^A-Z]", "", b4)
                                                                                                           
                        c1 = re.sub(r"[^0-9/^AM$/^AMD$/^인텔$/^소켓$]", "", spec)
                        c2 = c1.split('/')
                        c3 = [word for word in c2 if '소켓' in word]
                        c4 = '/'.join(c3)
                        board_socket = re.sub(r"[^0-9/^가-힣/^A-Z]", "", c4)
                        ...
                        insertcur = ("INSERT INTO board_Product (name, price, board_ddr, board_atx, board_socket, url, image) VALUES (%s, %s, %s, %s, %s, %s, %s)")
                        data = (name, price, board_ddr, board_atx, board_socket, url, image)
